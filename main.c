...
=======
    ...
    FoundSameDate = false ;
          ...
          while (temp_cur->next != NULL){//to check if the Attende is a single vistor  and have an reservation in the same dste
            ...
            while (temp_cur->next != NULL){//to check if the Attende is a registerd  and have an reservation in the same dste
            if( temp->id == temp_cur->id && temp_cur->date->dd == temp->date->dd && temp_cur->date->mm == temp->date->mm  && temp_cur->date->yy == temp->date->yy ){
            FoundSameDate = true ;
             break ;
             }
...
>>>>>>> Stashed changes
        ...
        for (i = 0; i < temp->numOfCompanion; i++) {
            ...
            else if( Ctemp.age < TempPName.ageRestriction ){// if the Companion's age dose not match the ageRestriction 
            printf("Sorry There is a ageRestriction in this event( age must be > %d  years )  .. try again ! ", TempPName.ageRestriction );
             ageRestriction =  true ;
             break  ; }
            }// end while
            ...
            *(temp->listOfCompanion + i) = Ctemp;
                ...
                temp->totalFees =
                        temp->numOfCompanion > 0 ? (temp->activityFee + ((temp->numOfCompanion  - counterfreePassAge ) * temp->activityFee)): temp->activityFee;
      ...
      void editRecord(){
         ...
         struct Attendee *temp_cur = singleVisit_head ;
          while (temp_cur->next != NULL){//to check if the Attende is a single vistor
            ...
            if( temp_cur->id == Attende_ID )//to check ifthe Attende is a single vistor and  there is just one node or this is the last node
            found = true ;
           ...
           printf("You do not have any companion .. So you can not edit this record");
            ...
            if( temp_cur->id == Attende_ID )//to check ifthe Attende is a registered vistor and  there is just one node or this is the last node
            found = true ;
            ...
            Companion *Temp_listOfCompanion =(Companion *) calloc( ( size_of_memoryloc  - counter_IDS ) ,sizeof(Companion)) ;
            ...
            Companion Ctemp;
            temp_cur->listOfCompanion =(Companion *) realloc(temp_cur->listOfCompanion,(temp_cur->numOfCompanion+NumberOfIDS)*sizeof(Companion)) ;
